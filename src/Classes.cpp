...
class Mesh {
    ...
    public:
        Mesh(string objPath, glm::vec3 p, float s) {
			...
			isSelected = false;
            textureFile = getTextureImage(ASSETS_DIRECTORY+materialFile);
            if(textureFile != "")	textureID = loadTexture(ASSETS_DIRECTORY+textureFile);
        }
...
int main(){
	glfwInit();
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Modulo 5 - Camera+Classes - Helena!", nullptr, nullptr);
    ...
    objects.push_back(Mesh(ASSETS_DIRECTORY+OBJECT_FILE, glm::vec3(0.5,0.0,0.0), 0.2));
    objects.push_back(Mesh(ASSETS_DIRECTORY+OBJECT_FILE, glm::vec3(-0.5,0.0,0.0), 0.2));
	...
	Material material(ASSETS_DIRECTORY+objects[0].materialFile);
...
void setupLights(int shaderID){
	lights.push_back(Light(glm::vec3(0.0, 0.0, -0.5), glm::vec3(1.0, 1.0, 1.0)));
	lights.push_back(Light(glm::vec3(0.0, 0.0, -1.0), glm::vec3(0.8, 0.8, 0.8)));
	lights.push_back(Light(glm::vec3(0.0, 1.0, 0.0), glm::vec3(1.0, 1.0, 1.0)));
